...
function createPostElement(post, isEditable){
  ...
  interactionDiv.className = "post-actions";
  ...
  interactionDiv.appendChild(likeBtn);

  // Comment Button
  var commentBtn = document.createElement("button");
  commentBtn.className = "btn-comment";
  commentBtn.textContent = "💬";
  interactionDiv.appendChild(commentBtn);
  ...
  // ====== Edit/Delete for Dashboard ======
  if(isEditable){
    var editDiv = document.createElement("div");
    editDiv.className = "post-actions";

    var editButton = document.createElement("button");
    editButton.className = "btn btn-edit";
    editButton.textContent = "Edit";
    editButton.onclick = function(){ editPost(post.id); };
...
function toggleLike(postId, likeBtn){
  ...
  for(var i=0;i<posts.length;i++){
    if(posts[i].id === postId){
      ...
      var idx = posts[i].likedBy.indexOf(currentUser.id);
      if(idx === -1){
        posts[i].likedBy.push(currentUser.id);
      ...
      localStorage.setItem("posts", JSON.stringify(posts));
      likeBtn.innerHTML = `❤ ${posts[i].likes}`;
      break;
    }
  }
}
...
// Update loadAllPosts function to include search + filter
function loadAllPosts() {
  var posts = JSON.parse(localStorage.getItem("posts")) || [];
...
function renderComments(postId, commentSection){
  ...
  post.comments.forEach((c, idx)=>{
    ...
    // Add delete button if current user is comment owner
